...
spectrum(diff(manzana_train),  spans = c(3, 3), main = "", xlab  = "Frecuencia", ylab = "Espectro", ci.col=NULL)
...
Para poner a prueba este supuesto se realiza una prueba con el estadístico t, donde se testea $H_0) \mu_{\varepsilon} = 0$ contra $H_1) \mu_{\varepsilon} = 0$. En el caso de la muestra no se rechaza la hipótesis nula al 99% de confianza, por lo que la evidencia está a favor de que su media incondicional es 0.
...
Con el test de Wilcoxon no rechazamos la hipótesis nula de que la media de los residuos es igual a 0, por lo cual este supuesto se cumple.
...
```{r, results='asis', fig.cap="Errores de los distintos modelos en la muestra de testeo."}
errores_test <- rbind(
...
Notar que el modelo intervenido por tso está sobreajustado (capaz)  y por eso cumple todos los supuestos tan bien pero predice mal.
