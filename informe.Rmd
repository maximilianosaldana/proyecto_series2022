...
Para cuantificar el error que se comete al comparar predicción contra valor verdadero, se recurre a distintas medidas de error, cada cual con sus particularidades. 

La raíz del error cuadrático medio  (RMSE) y el error absoluto medio (MAE) son dos medidas ampliamente usadas, debido a su relevancia en el modelado estadístico (generalmente se minimiza el RMSE para obtener las predicciones). Ambas medidas son dependientes de la escala, por lo que no pueden usarse para comparar el error de modelos que no están en la misma escala y además son sensibles a los outliers. Sea $\e_t = $  


```{r, results='asis', fig.cap="Errores de los distintos modelos en la muestra de entrenamiento."}
errores_train <- rbind(
