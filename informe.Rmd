...
lmtest::coeftest(modelo1) #Los coef ar1 y ma1 son mayores a 1 y similares en magnitud, posible sobreparametrización, probamos sacar el ma1

modelo1 <- Arima(manzana_train, order = c(2, 1, 0), seasonal = c(2, 0, 0), fixed=c(NA, NA, 0, NA))
...
p_valores_box2_int2 <- tibble(
  lag = 1:36, 
  p_valores = sapply(1:36, function(i) Box.test(r2_int2, lag = i, fitdf = 4 , type = "Ljung-Box")$p.value)
  )
...
Los parámetros resultantes de la estimación para el modelo manual, una vez intervenido, son:

* Parte autorregresiva: $\hat \phi_1 = 0.951$ y $\hat \phi_2 = -0.4463$.

* Parte medias móviles: $\hat \theta_1 = -0.4157$.

* Parte autorregresiva estacional: $\hat \Phi_1 = 0.102 $ y $\hat \Phi_2 = 0.397$.
