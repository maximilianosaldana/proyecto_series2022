...
toma el valor 0.51 (entre más cercano a 1 más fuerte el componente).

Esto refuerza la necesidad de considerar la estacionalidad a la hora de especificar un modelo. 
...
$$\rho(t, t+j) = \frac{Cov(X_t, X_{t+j})}{\sigma_t\sigma_{t+j}}  $$

Mientras que la función de autocorrelación parcial (PACF) es la autocorrelación entre  $X_t$ y $X_{t+j}$ una vez se quita el efecto de las correlaciones intermedias que hay entre ambas variables [@notas_series].
...
Teniendo esto en cuenta, inicialmente se plantea un modelo $ARIMA(2,1,1)(2,0,0)$
