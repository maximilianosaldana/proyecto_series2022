...
$$\hat{Y}_{t+s}|Y_t \pm z_{\alpha/2}\sqrt{E[Y_t+s-E(Y_{t+s}|\varepsilon_t,\varepsilon_{t-1},\dots)]^2} = \hat{Y}_{t+s}|Y_t \pm z_{\alpha/2}\sqrt{\sigma^2(1+\sum_{j=1}^{s-1}\Psi_j^2)}$$

Donde $\hat{Y}_{t+s}$ es la predicción puntual, $z_{\alpha/2}$ es el percentil $\alpha/2$ de una distribución normal, $\sigma^2$ es la varianza de los errores y $\Psi_j$ es el j-ésimo parámetro de la representación $AR(\infty)$ del modelo (que siempre es posible hallarla cuando el modelo es estacionario por el teorema de Wold [@notas_series]).
